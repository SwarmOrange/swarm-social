...
let webFullJs = fs.readFileSync('./js/dist/web-full.min.js', "utf8");
...
allCss = allCss
    .replace('../device-mockups/iPhone6/portrait_white.png', './device-mockups/iPhone6/portrait_white.png');

indexDev = indexDev.split('<script src="js/scripts.js"></script>').join('<script>' + webFullJs + '</script>');
indexDev = indexDev
    .replace('<link href="css/uglify.css" rel="stylesheet">', '')
    .replace('<link href="css/style.css" rel="stylesheet">', '')
    .replace('<!-- CSS auto paste here -->', '<style>' + allCss + '</style>');
